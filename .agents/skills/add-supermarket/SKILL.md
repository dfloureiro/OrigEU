...
- Update the authoritative supported-sites table and setup example in `README.md`, plus the site notes and file map in `ARCHITECTURE.md`. Keep `extDescription` and the detailed text in `docs/store-listings/` generic so adding a retailer does not create stale copy or keyword lists in store metadata. Brand origin records are managed through the backend; adding a site does not imply populating that database.
