...
- Before packaging or rollout, review `extDescription` in every `_locales/*/messages.json` against the extension's current behavior. Each translation should describe brand-level origin accurately in concise, generic wording. Review the detailed store descriptions in `docs/store-listings/` too: do not enumerate retailer names, domains, countries as search terms, or repeat feature terms for discoverability; describe coverage naturally and keep the authoritative site list in `README.md`. Check current store length and keyword-spam rules, then verify the finalized short descriptions inside both ZIPs; a valid manifest alone does not catch stale localized copy.
