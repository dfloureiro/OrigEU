...
(function () {
  ...
  function brandNote(result) {
    ...
    const byLang = { pt: sig.notesPt, es: sig.notesEs, fr: sig.notesFr, de: sig.notesDe, it: sig.notesIt, en: sig.notesEn };
    const preferred = lang.startsWith('pt') ? 'pt' : lang.startsWith('es') ? 'es' : lang.startsWith('fr') ? 'fr' : lang.startsWith('de') ? 'de' : lang.startsWith('it') ? 'it' : 'en';
    // Preferred language first, then fall back through the rest (en before
    // pt/es/fr/de/it as the most likely to be populated — see backend/README.md) so
    ...
    const order = [preferred, 'en', 'pt', 'es', 'fr', 'de', 'it'].filter((l, i, arr) => arr.indexOf(l) === i);
